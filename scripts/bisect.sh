...
DISTINCT="${DISTINCT:-1}"   # distinct pipelines cycled across the chain (pipeline-state switch axis)
...
for tag in ${VERSIONS}; do
  ...
  "/tmp/chainprobe-${tag}" -backend "${BACKEND}" -ksweep "${KS}" -n "${N}" -runs "${RUNS}" -bindings "${BINDINGS}" -distinct "${DISTINCT}" -csv \
