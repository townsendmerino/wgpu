...
| Minimize CGO crossings per dispatch | **Implemented** as `ComputePassEncoder.RecordSteps([]ComputeStep)` — an additive, non-cogentcore-mirroring fast path that records a whole dispatch batch in one CGO crossing (the loop runs in C). Measured **~5× faster recording: 85.8 → 16.7 ns/dispatch** (record-only), at the cost of ~3 transient slice allocations per call instead of per-dispatch crossings. Absolute saving is still only ~0.04 ms/token, and a caller must opt in (rewrite its record loop), so it does not help the cogentcore drop-in path — but it's there for hot recorders that want it. See [recordsteps_test.go](../recordsteps_test.go). |
